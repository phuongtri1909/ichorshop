                                ...
                                <div class="form-group">
                                    ...
                                    <div class="input-group">
                                        <input type="number" id="value" name="value" class="form-control"
                                            placeholder="Nhập giá trị giảm giá" value="{{ old('value', $coupon->value) }}"
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <input type="number" id="min_order_amount" name="min_order_amount"
                                            ...
                                            value="{{ old('min_order_amount', $coupon->min_order_amount) }}" min="0"
                                            step="0.01">
                                    </div>
                    ...
                    <div class="form-section">
                        ...
                        <div class="variant-selection">
                            <div class="form-group">
                                ...
                                @if (count($selectedVariantIds) > 0)
                                    <div class="alert alert-info mb-3">
                                        <i class="fas fa-info-circle me-2"></i>
                                        Đã chọn {{ count($selectedVariantIds) }} biến thể sản phẩm.
                                    </div>
                                @endif
                                ...
                                <div class="products-container" id="products-container">
                                    ...
                                    <div id="products-list">
                                        ...
                                        @if (isset($selectedProducts) && $selectedProducts->count() > 0)
                                            @foreach ($selectedProducts as $product)
                                                <div class="product-item mb-3">
                                                    <div class="product-header">
                                                        <div class="form-check">
                                                            <input class="form-check-input product-checkbox initialized"
                                                                type="checkbox" id="product_{{ $product->id }}"
                                                                data-product-id="{{ $product->id }}">
                                                            <label class="form-check-label"
                                                                for="product_{{ $product->id }}">
                                                                <span class="product-name">{{ $product->name }}</span>
                                                                <span class="product-sku text-muted">(SKU:
                                                                    {{ $product->sku }})</span>
                                                    ...
                                                    <div class="row mt-2" id="variants_{{ $product->id }}">
                                                        @foreach ($product->variants as $variant)
                                                            <div class="variant-item col-3">
                                                                <div class="form-check">
                                                                    <input
                                                                        class="form-check-input variant-checkbox initialized"
                                                                        type="checkbox" name="product_variants[]"
                                                                        value="{{ $variant->id }}"
                                                                    ...
                                                                    <label class="form-check-label"
                                                                        ...
                                                                        @if ($variant->color_name)
                                                                            <span
                                                                                class="variant-color">{{ $variant->color_name }}</span>
                                                                        @endif
                                                                        @if ($variant->size)
                                                                            <span class="variant-size">Size:
                                                                                {{ $variant->size }}</span>
                                                                        @endif
                                                                        <span
                                                                            class="variant-price">${{ $variant->price }}</span>
                                                                    </label>
                                                                </div>
                                                            </div>
                                                        @endforeach
            ...
            function markSelectedVariants() {
                if (selectedVariantIds.length > 0) {
                    selectedVariantIds.forEach(id => {
                        const variantCheckbox = $(`#variant_${id}`);
                        if (variantCheckbox.length) {
                            variantCheckbox.prop('checked', true).trigger('change');
                        }
                    });
                }
            }
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        Swal.fire({
                            icon: 'success',
                            title: 'Thành công',
                            text: response.message || 'Mã giảm giá đã được cập nhật thành công',
                            timer: 2000,
