<div class="variant-item mb-3 form-card p-2" data-index="{{ $index }}">
    <div class="card-header d-flex justify-content-between align-items-center py-2">
        ...
        <button type="button" class="btn btn-sm btn-outline-danger remove-variant-btn {{ $index == 0 ? 'd-none' : '' }}"
    ...
    <div class="card-body">
        <div class="row">
            ...
            <div class="form-group col-md-4">
                    ...
                    <div class="color-toggle-wrapper mb-2">
                        <div class="form-check form-switch">
                            <input class="form-check-input color-toggle" type="checkbox"
                                id="colorToggle{{ $index }}"
                                {{ !isset($variant->color) || empty($variant->color) ? 'checked' : '' }}>
                            <label class="form-check-label" for="colorToggle{{ $index }}">Không màu</label>
                        </div>
                    </div>
                    <div class="color-inputs"
                        {{ !isset($variant->color) || empty($variant->color) ? 'style="display: none;"' : '' }}>
                        <input type="color" name="variants[{{ $index }}][color]"
                            class="color-picker color-input" value="{{ $variant->color ?? '' }}" title="Chọn màu">
                ...
                @if (isset($errors) && $errors->has("variants.$index.color_name"))
                    <div class="invalid-feedback">{{ $errors->first("variants.$index.color_name") }}</div>
                @endif
            </div>
            ...
            <div class="form-group col-md-4">
                ...
                <select name="variants[{{ $index }}][status]" class="custom-input">
                    ...
                    <option value="active" {{ ($variant['status'] ?? 'active') == 'active' ? 'selected' : '' }}>Hoạt
                        động</option>
                    <option value="inactive" {{ ($variant['status'] ?? '') == 'inactive' ? 'selected' : '' }}>Không
                        hoạt động</option>
                </select>
        ...
        <div class="row">
            ...
            <div class="form-group col-md-4">
                ...
                <input type="number" name="variants[{{ $index }}][quantity]" class="custom-input"
                    placeholder="0" min="0"
                    value="{{ $variant['quantity'] ?? ($variant['stock_quantity'] ?? '') }}">
